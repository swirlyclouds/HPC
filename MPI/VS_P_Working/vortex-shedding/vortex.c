    ...
    for (iter = 0; iter < itermax; iter++) {
        for (int rb = 0; rb < 2; rb++) {
            ...
            if(size > 1 && size < 4){
                ...
                // {
                    if(rank != 0){
                    MPI_Send(&p[start][0], p_size_y, MPI_DOUBLE, rank-1, 3, MPI_COMM_WORLD);
                    MPI_Recv(&(p[start - 1][0]), p_size_y, MPI_DOUBLE, rank-1, 0, MPI_COMM_WORLD, &status);
                    }
                    if(size > rank + 1){
                        // printf("sending...\n");
                        MPI_Send(&(p[end-1][0]), p_size_y, MPI_DOUBLE, rank+1, 1, MPI_COMM_WORLD);
                        MPI_Recv(&p[end][0], p_size_y, MPI_DOUBLE, rank+1, 2, MPI_COMM_WORLD, &status);
                    }
                // }
                // if(rank == 2){
